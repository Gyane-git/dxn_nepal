...
import { resolveFonepayConfig, verifyFonepayPayment } from "@/lib/fonepay";
import {
  applyCoupon,
  computeSubtotal,
...
/**
 * The order is only ever created here, once Fonepay's verification API — the
 * authoritative source of truth, never the PRN/BID/UID/P_AMT query params Fonepay
 * appends to the return URL — confirms the payment succeeded.
 */
export async function POST(request: Request) {
  try {
    ...
    const bid: string | undefined = body.bid;
    const uid: string | undefined = body.uid;
    if (!transactionUuid || !bid || !uid) return fail(400, "Missing transaction reference");
    ...
    const result = await verifyFonepayPayment({ prn: transactionUuid, bid, uid, amount: total }, config);
    ...
    const order = await createOrderFromCart({
      ...
      paymentSubMethod: "FONEPAY",
      paymentStatus: "PAID",
      paymentReference: bid,
      historyNote: `Order placed — paid via Fonepay (ref: ${bid})`,
    });
