...
function KhaltiReturnContent() {
  ...
  useEffect(() => {
    async function complete() {
      ...
      if (!txn || !pidx || !raw) {
        setMessage(
          `We couldn't find your checkout details in this browser session. If Khalti charged you, please check My Orders or contact support with reference ${txn ?? "unknown"}.`
        );
        return;
      }
      ...
      const res = await fetch("/api/checkout/khalti/complete", {
        method: "POST",
  ...
  if (!message) {
    return (
      <div className="mx-auto max-w-md px-4 py-24 text-center">
        ...
        <p className="mt-4 text-gray-500">Confirming your Khalti payment…</p>
      </div>
    );
  }
